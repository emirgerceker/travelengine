...
if (!class_exists('Togo_Breadcrumb')) {

	/**
	 *  Class Togo_Breadcrumb
	 */
	class Togo_Breadcrumb
	{
		...
		public static function breadcrumb($args = '')
		{
			...
			$default = array(
				'type'             => 'list',
				'home_label'       => esc_html__('Home', 'togo'),
				'search_label'     => esc_html__('Search Result of "%s"', 'togo'),
				'404_label'        => esc_html__('404 Not Found', 'togo'),
				'category_label'   => _x('%s', 'category label', 'togo'),
				'tag_label'        => _x('%s', 'tag label', 'togo'),
				'taxonomy_label'   => _x('%s', 'taxonomy label', 'togo'),
				'author_label'     => _x('%s', 'author label', 'togo'),
				'attachment_label' => _x('%s', 'attachment label', 'togo'),
				'year_label'       => _x('%s', 'year label', 'togo'),
				'month_label'      => _x('%s', 'month label', 'togo'),
				'day_label'        => _x('%s', 'day label', 'togo'),
				'post_type_label'  => _x('%s', 'post type label', 'togo'),
				'joint_string'     => esc_html__(' &gt; ', 'togo'),
				'navi_element'     => '',
				'elm_class'        => 'togo-breadcrumb',
				'elm_id'           => '',
				'li_class'         => '',
				'class_prefix'     => '',
			...
			$default = apply_filters('togo_breadcrumb_default', $default);
			...
			$args['elm_class']     = self::sanitize_attr_classes($args['elm_class'], $args['class_prefix']);
			$args['li_class']      = self::sanitize_attr_classes($args['li_class'], $args['class_prefix']);
			$args['current_class'] = self::sanitize_attr_classes($args['current_class'], $args['class_prefix']);
			$args['current_class'] = $args['current_class'] ? $args['current_class'] : $args['class_prefix'] . $default['current_class'];
			$args['echo']          = self::uniform_boolean($args['echo'], $default['echo']);

			$tabs = str_repeat("\t", (int) $args['indent']);

			$breadcrumb_arr = self::get_breadcrumb_array($args);
			...
			if ($args['type'] == 'string') {
				...
				$output = apply_filters('togo_breadcrumb_after', $output, $args);
			} else {
				...
				$output = apply_filters('togo_breadcrumb_before', $output, $args);
			...
			$output = apply_filters('togo_breadcrumb', $output, $args);
		...
		public static function get_breadcrumb_array($args)
		{
			global $post;

			$breadcrumb_arr   = array();
			$breadcrumb_arr[] = array('title' => $args['home_label'], 'link' => home_url() . '/');
			$breadcrumb_arr   = self::add_posts_page_array($breadcrumb_arr);
			if (is_404()) {
				...
				if ($post->post_parent) {
					if ($parent_post = get_post($post->post_parent)) {
						$singular_breadcrumb_arr = self::get_singular_breadcrumb_array($parent_post, $args);
				...
				$singular_breadcrumb_arr = self::get_singular_breadcrumb_array($post, $args);
				$breadcrumb_arr          = array_merge($breadcrumb_arr, $singular_breadcrumb_arr);
			...
			return apply_filters('togo_breadcrumb_arr', $breadcrumb_arr, $args);
		}

		public static function get_singular_breadcrumb_array($post, $args)
		{
			$breadcrumb_arr = array();
		...
		public static function add_posts_page_array($breadcrumb_arr)
		{
		...
		public static function sanitize_attr_classes($classes, $prefix = '')
		{
			if (! is_array($classes)) {
		...
		public static function uniform_boolean($arg, $default = true)
		{
			if (is_numeric($arg)) {
				$arg = (int) $arg;
			}
