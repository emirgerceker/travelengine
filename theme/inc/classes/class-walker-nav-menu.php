<?php
class Togo_Walker_Nav_Menu extends Walker_Nav_Menu
{
	...
	public function start_el(&$output, $item, $depth = 0, $args = array(), $id = 0)
	{
		...
		foreach ($children as $child) {
			...
			if ($obj[0] === 'togo_mega_menu') {
				$classes[]       = apply_filters('togo_mega_menu_css_class', 'has-mega-menu', $item, $args, $depth);
				$this->mega_menu = true;
			}
		}
		...
		if ($args->has_children) {
			$item_output .=  \Togo\Icon::get_svg('chevron-down');
		}

		$item_output .= '</a>';
		$item_output .= $args->after;

		if ('togo_mega_menu' === $item->object && defined('ELEMENTOR_VERSION')) {
			$mega_menu_content_class = apply_filters('togo_mega_menu_content_css_class', 'mega-menu-content', $item, $args, $depth);
