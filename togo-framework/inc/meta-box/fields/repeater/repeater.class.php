...
if ( !class_exists('Uxper_Field_Repeater') ) {
	class Uxper_Field_Repeater extends Uxper_Field
	{
		function html_start()
		{
			$wrapper_class = 'uxper-field uxper-repeater';
			if (isset($this->params['sort']) && $this->params['sort']) {
				$wrapper_class .= ' uxper-repeater-sort';
			}
		...
		function html_repeater_desc() {
			...
			?>
			<div class="uxper-row uxper-repeater-footer">
				<?php foreach ($this->params['fields'] as $field): ?>
					<?php
					$col = isset($this->params['col']) ? $this->params['col'] : 12;
					$col = isset($field['col']) ? $field['col'] : $col;
					?>
					<div class="uxper-col uxper-col-<?php echo esc_attr($col); ?>">
						<p class="uxper-desc"><?php echo (isset($field['desc']) ? $field['desc'] : ''); ?></p>
					</div>
				<?php endforeach;?>
			</div><!-- /.uxper-row -->
			<?php
		...
		function html_repeater_label() {
			?>
			<div class="uxper-row uxper-repeater-header">
				<?php foreach ($this->params['fields'] as $field): ?>
					<?php
						$col = isset($this->params['col']) ? $this->params['col'] : 12;
						$col = isset($field['col']) ? $field['col'] : $col;
					?>
					<div class="uxper-col uxper-col-<?php echo esc_attr($col); ?>">
						<label class="uxper-label"><?php echo (isset($field['title']) ? $field['title'] : ''); ?></label>
					</div>
				<?php endforeach;?>
			</div><!-- /.uxper-row -->
			<?php
		...
		function html_content() {
			...
			?>
				<div class="uxper-field-content-wrap">
					...
					<div class="<?php echo esc_attr($content_wrap_class); ?>">
						...
						<?php for ($i = 0; $i < $count; $i++): ?>
							<div class="uxper-row uxper-field-content uxper-field-content-repeater uxper-clone-field">
