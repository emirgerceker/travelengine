...
if (!class_exists('Uxper_Field_Panel')) {
	class Uxper_Field_Panel extends Uxper_Field
	{
		function enqueue()
		{
			wp_enqueue_script(TOGO_METABOX_PREFIX . 'panel', TOGO_FRAMEWORK_DIR . 'inc/meta-box/fields/panel/assets/panel.js', array(), null, true);
		}
...
?>
			<div id="<?php echo esc_attr($field_id); ?>" class="uxper-field-panel uxper-field" <?php $this->the_required(); ?>>
				<div class="uxper-field-panel-inner">
				<?php
			}
			function html_end()
			{
				?>
				</div><!-- /.uxper-field-panel-inner -->
			</div><!-- /.uxper-field-panel -->
		<?php
			}
			...
			function html_content()
			{
				$count = 0;
				if ($this->is_clone()) {
					$count = $count = apply_filters('uxper_' . uxper_get_config_type() . '_get_panel_count', $count, $this);
				}
				$isToggle = isset($this->params['toggle']) ? $this->params['toggle'] : true;
		?>
			<div class="uxper-field-content-wrap <?php echo esc_attr($this->is_sort() ? 'uxper-field-panel-sortable' : ''); ?>">
				<?php
				...
				<div class="<?php echo esc_attr($content_wrap_class); ?>">
					<?php for ($i = 0; $i < $count; $i++): ?>
						<div class="uxper-field-content uxper-clone-field uxper-clone-field-panel"
							data-panel-index="<?php echo esc_attr($i); ?>" data-panel-id="<?php echo esc_attr($this->get_id()); ?>">
							<?php if (!empty($this->params['title'])): ?>
								<h4 class="uxper-field-panel-title">
									<span class="uxper-panel-title" data-label="<?php echo esc_attr($this->params['title']); ?>"><?php echo esc_html($this->params['title']); ?></span>
									<?php if ($isToggle): ?>
							...
							<?php endif; ?>
							<div class="uxper-clone-field-panel-inner"
								<?php if ($isToggle && !(isset($this->params['toggle_default']) ? $this->params['toggle_default'] : true)): ?>
								style="display: none"
								<?php endif; ?>>
