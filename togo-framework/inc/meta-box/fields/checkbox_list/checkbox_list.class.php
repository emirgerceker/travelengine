...
if (!class_exists('Uxper_Field_Checkbox_List')) {
	class Uxper_Field_Checkbox_List extends Uxper_Field
	{
		function enqueue()
		{
			wp_enqueue_script(TOGO_METABOX_PREFIX . 'checkbox_list', TOGO_FRAMEWORK_DIR . 'inc/meta-box/fields/checkbox_list/assets/checkbox-list.js', array(), null, true);
		}
...
?>
			<div class="uxper-field-checkbox_list-inner <?php echo ($value_inline ? 'value-inline' : ''); ?>">
				<?php foreach ($this->params['options'] as $key => $value): ?>
					<label>
						<input data-field-control="" class="uxper-checkbox_list" type="checkbox" name="<?php echo esc_attr($this->get_name()) ?>[]" value="<?php echo esc_attr($key); ?>" <?php echo (in_array($key, $field_value) ? ' checked="checked"' : ''); ?> />
