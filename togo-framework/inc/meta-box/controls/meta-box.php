 ...
 *
 */
if (!class_exists('Uxper_Meta_Box')) {

	class Uxper_Meta_Box
	{
		...
		public function __construct()
		{
			...
			add_filter('uxper_meta_box_get_value', array($this, 'meta_box_get_value'), 10, 2);
			add_filter('uxper_meta_box_get_clone_count', array($this, 'meta_box_get_clone_count'), 10, 2);
			add_filter('uxper_meta_box_get_panel_count', array($this, 'meta_box_get_panel_count'), 10, 2);
		}
		...
		public function admin_enqueue_scripts()
		{
			...
			wp_enqueue_script(TOGO_METABOX_PREFIX . 'field_config', TOGO_FRAMEWORK_DIR . 'inc/meta-box/assets/js/field-config.js', array(), false, true);
		...
		{
			$meta_configs = &uxper_get_meta_boxes_config();
		...
		public function meta_box_display_callback($post, $args)
		{
			...
			uxper_get_template('templates/meta-box-start', array('list_section' => $list_section));
			if (isset($config['section'])) {
				$tab_index = 0;
				foreach ($config['section'] as $tabs) {
					echo sprintf('<div id="section_%s" class="uxper-section-container" %s>', $tabs['id'], $tab_index == 0 ? 'style="display:block"' : 'style="display:none"');
					if ($tabs['id'] == 'trip_schedule_tabs') {
						do_action('togo_trip_schedule_tabs');
					}
		...
		{
			if (uxper_is_edit_page('new')) {
				return false;
			}
			if (!isset($GLOBALS['uxper_db_meta_key_' . $post_id])) {
				$GLOBALS['uxper_db_meta_key'] = array();
				global $wpdb;
				$rows = $wpdb->get_results($wpdb->prepare("SELECT meta_key FROM $wpdb->postmeta WHERE post_id = %d", $post_id));
				foreach ($rows as $row) {
					$GLOBALS['uxper_db_meta_key_' . $post_id][] = $row->meta_key;
				}
			}

			return in_array($meta_key, $GLOBALS['uxper_db_meta_key_' . $post_id]);
		}
