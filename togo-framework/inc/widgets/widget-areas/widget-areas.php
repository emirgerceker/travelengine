 ...
 * @since emo 1.0
 */
if (!class_exists('TOGO_Widget_Areas')) {
	class TOGO_Widget_Areas
	{
		...
		public function __construct()
		{
			if (is_admin()) {
				...
				add_action('wp_ajax_togo_delete_widget_area', array($this, 'delete_widget_area'));
			}
		...
		{
			wp_enqueue_script('togo-widget-areas', plugins_url(TOGO_FRAMEWORK_DIR . '/inc/widgets/widget-areas/assets/js/widget-areas.min.js'), array('jquery'), $this->version);
			wp_enqueue_style('togo-widget-areas', plugins_url(TOGO_FRAMEWORK_DIR . '/inc/widgets/widget-areas/assets/css/widget-areas.min.css'), array(), $this->version, 'screen');
			wp_localize_script(
				'togo-widget-areas',
				'togo_widget_areas_variable',
				array(
					'ajax_url' => admin_url('admin-ajax.php'),
					'confirm_delete' => esc_html__('Are you sure to delete this widget areas?', 'togo')
				)
			);
		}
		...
		public function register_custom_widget_areas()
		{
			...
			$args = apply_filters('togo_custom_widget_args', $args);
