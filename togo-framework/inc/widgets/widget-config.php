 ...
 */
abstract class Togo_Widget extends WP_Widget
{
	...
	public function __construct()
	{
		...
		add_action('save_post', array($this, 'togo_widget_cache'));
		add_action('deleted_post', array($this, 'togo_widget_cache'));
		add_action('switch_theme', array($this, 'togo_widget_cache'));
	...
	{
		// base
		wp_enqueue_style('togo_widget', TOGO_FRAMEWORK_DIR . '/inc/widgets/assets/css/widget.css', array(), false, 'all');
		wp_enqueue_script('togo_widget', TOGO_FRAMEWORK_DIR . '/inc/widgets/assets/js/widget.js', array(), false, true);
	}
	...
	public function cache_widget($args, $content)
	{
		...
		$cache = wp_cache_get($this->widget_id, 'widget') ? wp_cache_get($this->widget_id, 'widget') : array();
	 ...
	 * @return void
	 */
	public function togo_widget_cache()
	{
...
?>
				...
				?>
					<p>
						<label for="<?php echo esc_attr($this->get_field_id($key)); ?>"><?php echo esc_html($setting['label']); ?></label>
						<input name="<?php echo esc_attr($this->get_field_name($key)); ?>" type="hidden" value="<?php echo esc_attr($value); ?>" />
						<select multiple="multiple" class="widefat togo-select2" id="<?php echo esc_attr($this->get_field_id($key)); ?>" data-value="<?php echo esc_attr($value) ?>">
							<?php foreach ($setting['options'] as $option_key => $option_value) : ?>
				...
				?>
					<div>
						...
						<div>
							...
							<input style="float: right" title="<?php echo esc_attr__('Click to browse icon', 'togo') ?>" class="browse-icon button-secondary" type="button" value="<?php echo esc_attr__('Browse...', 'togo') ?>" />
				...
				?>
					<div style="margin: 13px 0">
						...
						<div class="widget-image-field">
							...
							<button style="margin-top: 10px" title="<?php echo esc_attr__('Click to browse image', 'togo') ?>" class="browse-image button-secondary" type="button"><?php echo esc_html__('Upload image', 'togo') ?></button>
						</div>
					</div>
