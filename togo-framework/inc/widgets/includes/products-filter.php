<?php
if (!class_exists('Togo_Widget_Products_Filter')) {
    class Togo_Widget_Products_Filter extends \WP_Widget
    {
        ...
        public function __construct()
        {
            ...
            parent::__construct(
                'togo-products-filter',
                esc_html__('Togo - Products Filter', 'togo-framework'),
                array(
                    'classname'                   => 'products-filter-widget woocommerce',
                    'description'                 => esc_html__('WooCommerce products filter.', 'togo-framework'),
        ...
        public function widget($args, $instance)
        {
            ...
            echo '<div class="products-filter-activated">';
            ...
            if (! empty($instance['filter'])) {
                ...
                echo '<div class="products-filter-wrappers filters">';
                    ...
                    switch ($filter['source']) {
                        ...
                        case 'price':
                            ...
                            $list[] = sprintf(
                                ...
                                esc_attr($price),
                                esc_html__('Price', 'togo-framework'),
                                $price,
                                \Togo\Icon::get_svg('close')
                            );
                            $text = '';
                            break;
                        ...
                        case 'rating':
                            $text = _n('Rated %d star', 'Rated %d stars', $term, 'togo-framework');
        ...
        public function display_filter($filter)
        {
            ...
            if ('attribute' == $filter['source']) {
                ...
                $args['all']        = sprintf(esc_html__('Any %s', 'togo-framework'), wc_attribute_label($attr->attribute_label));
                ...
                $args['all'] = sprintf(esc_html__('Select a %s', 'togo-framework'), $taxonomy->labels->singular_name);
            } else {
                $args['all'] = esc_html__('All Products', 'togo-framework');
            }
            ...
            // CSS classes.
            $classes   = array('products-filter-wrapper', 'filter');
...
?>
            ...
            <div class="<?php echo esc_attr(join(' ', $classes)) ?>">
                <?php if (! empty($filter['name'])) : ?>
                    <span class="products-filter-wrapper-name filter-name"><?php echo esc_html($filter['name']) ?></span>
                <?php endif; ?>

                <div class="products-filter-wrapper-control filter-control">
                    <?php
        ...
        protected function get_filter_options($filter)
        {
            ...
            switch ($filter['source']) {
                ...
                case 'products_group':
                    $filter_groups = apply_filters('togo_products_filter_groups', array(
                        'best_sellers' => esc_html__('Best Sellers', 'togo-framework'),
                        'new'          => esc_html__('New Products', 'togo-framework'),
                        'sale'         => esc_html__('Sale Products', 'togo-framework'),
                        'featured'     => esc_html__('Hot Products', 'togo-framework'),
                    ));

                    if ('dropdown' != $filter['display']) {
                        $options[''] = array(
                            'name'  => esc_html__('All Products', 'togo-framework'),
                            'count' => 0,
                            'id'    => 0,
                            'level' => 0,
                        );
                ...
                case 'orderby':
                    $orderby = apply_filters('togo_products_filter_orderby', array(
                ...
                case 'product_status':
                    if (! isset($filter['disable_onsale'])) {
                        $options['on_sale'] = array(
                            'name'  => esc_html__('On Sale', 'togo-framework'),
                            'count' => 0,
                            'id'    => 'on_sale',
                            'level' => 0,
                        );
                    ...
                    if (! isset($filter['disable_in_stock'])) {
                        $options['in_stock'] = array(
                            'name'  => esc_html__('In Stock', 'togo-framework'),
                            'count' => 0,
                            'id'    => 'in_stock',
                            'level' => 0,
                        );
                ...
                default:
                    ...
                    $terms       = self::get_terms_hierarchy($taxonomy, '');
                    $query_type  = isset($filter['query_type']) ? $filter['query_type'] : 'and';
                    $term_counts = $this->get_filtered_term_product_counts(wp_list_pluck($terms, 'term_id'), $taxonomy, $query_type);
                    $categories_count = [];
        ...
        protected function filter_search_box($filter)
        {
            if ('attribute' == $filter['source']) {
                ...
                $placeholder = __('Search', 'togo-framework') . ' ' . strtolower($attributes[$filter['attribute']]);
            } else {
                $sources     = $this->get_filter_source_options();
                $placeholder = __('Search', 'togo-framework') . ' ' . strtolower($sources[$filter['source']]);
            }

            if ('dropdown' == $filter['display']) {
                printf(
                    '<span class="products-filter-search-box screen-reader-text">%s</span>',
                    esc_attr($placeholder)
                );
            } else {
                printf(
                    '<div class="products-filter-search-box"><input type="text" class="search-field" placeholder="%s" ></div>',
                    esc_attr($placeholder)
                );
            }
        }
        ...
        protected function display_ranges($args)
        {
            ...
            echo '<ul class="products-filter-options products-filter-ranges filter-ranges">';
            foreach ($args['options'] as $option) {
                printf(
                    '<li class="products-filter-option filter-ranges-item %s" data-value="%s"><span class="products-filter-option-name name">%s</span>%s</li>',
                    $args['current']['min'] == $option['range']['min'] && $args['current']['max'] == $option['range']['max'] ? 'selected' : '',
                    esc_attr(json_encode($option['range'])),
                    $option['name'],
                    $args['show_counts'] ? '<span class="products-filter-count count">' . $option['count'] . '</span>' : ''
                );
            }
            echo '</ul>';
        ...
        protected function display_list($args)
        {
            ...
            echo '<ul class="products-filter-options products-filter-list filter-list">';
            foreach ($args['options'] as $slug => $option) {
                ...
                printf(
                    '<li class="products-filter-option filter-list-item %s" data-value="%s"><span class="products-filter-option-name name">%s</span>%s%s',
                    esc_attr($class),
                    esc_attr($slug),
                    wp_kses_post(str_replace("&nbsp;", '', $option['name'])),
                    $args['show_counts'] ? '<span class="products-filter-count count">' . $option['count'] . '</span>' : '',
                    ! empty($option['has_children']) ? '<span class="products-filter-option-toggler" aria-hidden="true"></span>' : ''
                );
        ...
        protected function display_checkboxes($args)
        {
            ...
            echo '<ul class="products-filter-options products-filter-checkboxes filter-checkboxes">';
            foreach ($args['options'] as $slug => $option) {
                $class = in_array($slug, (array) $args['current']) ? 'selected' : '';
                ...
                printf(
                    '<li class="products-filter-option filter-checkboxes-item %s" data-value="%s"><span class="products-filter-option-name name">%s</span>%s%s',
                    esc_attr($class),
                    esc_attr($slug),
        ...
        protected function display_dropdown($args)
        {
            $args = wp_parse_args($args, array(
                ...
                'current'     => array(),
                'options'     => array(),
                'all'         => esc_html__('Any', 'togo-framework'),
                'show_counts' => false,
            ));
        ...
        protected function display_auto($args)
        {
            ...
            switch ($args['type']) {
                case 'color':
                    echo '<div class="products-filter-options products-filter-swatches filter-swatches">';

                    foreach ($args['options'] as $slug => $option) {
                        $color = $this->get_attribute_swatches($option['id'], 'color');

                        printf(
                            '<span class="products-filter-option swatch swatch-color swatch-%s %s" data-value="%s"  title="%s"><span class="bg-color" style="background-color:%s;"></span>%s%s</span>',
                            esc_attr($slug),
                ...
                case 'image':
                    ...
                    foreach ($args['options'] as $slug => $option) {
                        ...
                        printf(
                            '<span class="products-filter-option swatch swatch-image swatch-%s %s" data-value="%s" title="%s"><img src="%s" alt="%s">%s%s</span>',
                            esc_attr($slug),
                            in_array($slug, (array) $args['current']) ? 'selected' : '',
                            esc_attr($slug),
                            esc_attr($option['name']),
                ...
                case 'label':
                    ...
                    foreach ($args['options'] as $slug => $option) {
                        ...
                        printf(
                            '<span class="products-filter-option swatch swatch-label swatch-%s %s" data-value="%s" title="%s">%s%s</span>',
                            esc_attr($slug),
                            in_array($slug, (array) $args['current']) ? 'selected' : '',
                            esc_attr($slug),
                            esc_attr($option['name']),
                ...
                case 'button':
                    ...
                    foreach ($args['options'] as $slug => $option) {
                        ...
                        printf(
                            '<span class="products-filter-option swatch swatch-button swatch-%s %s" data-value="%s" title="%s">%s%s</span>',
                            esc_attr($slug),
                            in_array($slug, (array) $args['current']) ? 'selected' : '',
                            esc_attr($slug),
                            esc_attr($option['name']),
        ...
        public function form($instance)
        {
            ...
            $this->setting_field(array(
                'type'  => 'text',
                'name'  => 'title',
                'label' => esc_html__('Title', 'togo-framework'),
                'value' => $instance['title'],
            ));
        ?>

            <div class="togo-products-filter-form__sub-nav">
                <button type="button" data-section="filters"
                    class="button-link active"><?php esc_html_e('Filters', 'togo-framework'); ?></button>
                |
                <button type="button" data-section="options"
                    class="button-link"><?php esc_html_e('Options', 'togo-framework'); ?></button>
            </div>
            ...
            <div class="togo-products-filter-form__section active" data-section="filters">
                <p class="togo-products-filter-form__message <?php echo ! empty($instance['filter']) ? 'hidden' : '' ?>"><?php esc_html_e('There is no filter yet.', 'togo-framework') ?></p>

                <div class="togo-products-filter-form__filter-fields">
                    <?php $this->filter_setting_fields($instance['filter']); ?>
                </div>

                <p class="togo-products-filter-form__section-actions">
                    <button type="button" class="togo-products-filter-form__add-new button-link"
                        data-name="<?php echo esc_attr($this->get_field_name('filter')); ?>"
                        data-count="<?php echo count($instance['filter']) ?>">
                        + <?php esc_html_e('Add a new filter', 'togo-framework') ?></button>
                </p>
            </div>

            <div class="togo-products-filter-form__section" data-section="design">
            </div>

            <div class="togo-products-filter-form__section" data-section="options">
                <?php
                $this->setting_field(array(
                    'type'  => 'checkbox',
                    'name'  => 'ajax',
                    'label' => esc_html__('Use ajax for filtering', 'togo-framework'),
                    'value' => $instance['ajax'],
                ));
                ...
                $this->setting_field(array(
                    'type'      => 'checkbox',
                    'name'      => 'instant',
                    'label'     => esc_html__('Filtering products instantly (no buttons required)', 'togo-framework'),
                ...
                $this->setting_field(array(
                    'type'      => 'select',
                    'name'      => 'filter_buttons',
                    'label'     => esc_html__('Filter buttons position', 'togo-framework'),
                    'value'     => $instance['filter_buttons'],
                    'options'   => array(
                        'form'   => __('Bottom of form', 'togo-framework'),
                        'fitems' => __('Bottom of filter items', 'togo-framework'),
                    ),
                    'condition' => array(
                        'instant' => false,
                    ),
                ...
                $this->setting_field(array(
                    'type'      => 'checkbox',
                    'name'      => 'change_url',
                    'label'     => esc_html__('Update URL', 'togo-framework'),
        ...
        protected function filter_setting_fields($fields = array(), $context = 'display')
        {
            ...
            ?>
                <div class="togo-products-filter-form__filter">
                    <div class="togo-products-filter-form__filter-top">
                        <div class="togo-products-filter-form__filter-actions">
                            <button type="button"
                                class="togo-products-filter-form__remove-filter button-link button-link-delete">
                                <span class="screen-reader-text"><?php esc_html_e('Remove filter', 'togo-framework') ?></span>
                                <span class="dashicons dashicons-no-alt"></span>
                            </button>
                        </div>

                        <button type="button" class="togo-products-filter-form__filter-toggle">
                            <span class="togo-products-filter-form__filter-toggle-indicator" aria-hidden="true"></span>
                        </button>

                        <div class="togo-products-filter-form__filter-title"><?php echo $title; ?></div>
                    </div>
                    <div class="togo-products-filter-form__filter-options">
                        <?php
                        foreach ($filter_settings as $name => $options) {
                            ...
                            $options['class']      = 'togo-products-filter-form__filter-option';
        ...
        {
            $sources = array(
                'orderby'        => esc_html__('Order By', 'togo-framework'),
                'products_group' => esc_html__('Group', 'togo-framework'),
                'price'          => esc_html__('Price', 'togo-framework'),
                'attribute'      => esc_html__('Attributes', 'togo-framework'),
                'rating'         => esc_html__('Rating', 'togo-framework'),
                'product_status' => esc_html__('Product Status', 'togo-framework'),
            );
        ...
        protected function get_filter_display_options($source = 'product_cat')
        {
            $options = array(
                'price'     => array(
                    'slider' => esc_html__('Slider', 'togo-framework'),
                    'ranges' => esc_html__('Ranges', 'togo-framework'),
                ),
                'attribute' => array(
                    'auto'       => esc_html__('Auto', 'togo-framework'),
                    'dropdown'   => esc_html__('Dropdown', 'togo-framework'),
                    'list'       => esc_html__('Vertical List', 'togo-framework'),
                    'h-list'     => esc_html__('Horizontal List', 'togo-framework'),
                    'checkboxes' => esc_html__('Checkbox List', 'togo-framework'),
                ),
                'rating'    => array(
                    'dropdown'   => esc_html__('Dropdown', 'togo-framework'),
                    'checkboxes' => esc_html__('Checkbox List', 'togo-framework'),
        ...
        protected function get_filter_fields_settings()
        {
            ...
            $this->filter_settings = array(
                'name'        => array(
                    'type'  => 'text',
                    'label' => __('Filter Name', 'togo-framework'),
                ),
                'source'      => array(
                    'type'    => 'select',
                    'label'   => __('Filter By', 'togo-framework'),
                    'options' => $this->get_filter_source_options(),
                ),
                'attribute'   => array(
                    'type'      => 'select',
                    'name'      => 'attribute',
                    'label'     => __('Attribute', 'togo-framework'),
                ...
                'display'     => array(
                    'type'    => 'select',
                    'label'   => __('Display Type', 'togo-framework'),
                    'options' => $this->get_filter_display_options(),
                ),
                'ranges'      => array(
                    'type'      => 'textarea',
                    'label'     => __('Ranges', 'togo-framework'),
                    'desc'      => __('Each range on a line, separate by the <code>-</code> symbol. Do not include the currency symbol.', 'togo-framework'),
                    'condition' => array(
                        'display' => 'ranges',
                        'source'  => 'price',
                    ),
                ...
                'multiple'    => array(
                    'type'      => 'select',
                    'label'     => __('Selection Type', 'togo-framework'),
                    'options'   => array(
                        0 => __('Single select', 'togo-framework'),
                        1 => __('Multiple select', 'togo-framework'),
                    ),
                    'condition' => array(
                ...
                'query_type'  => array(
                    'type'      => 'select',
                    'label'     => __('Query Type', 'togo-framework'),
                    'options'   => array(
                        'and' => __('AND', 'togo-framework'),
                        'or'  => __('OR', 'togo-framework'),
                    ),
                    'condition' => array(
                        'source' => 'attribute',
                    ),
                ...
                'collapsible' => array(
                    'type'      => 'checkbox',
                    'label'     => __('Collapsible', 'togo-framework'),
                    'condition' => array(
                ...
                'show_counts' => array(
                    'type'      => 'checkbox',
                    'label'     => __('Show product counts', 'togo-framework'),
                    'condition' => array(
                ...
                'searchable'  => array(
                    'type'      => 'checkbox',
                    'label'     => __('Show the search box', 'togo-framework'),
                    'condition' => array(
                        'source!'  => array('product_status'),
                        'display!' => array('slider', 'ranges', 'auto'),
                    ),
                ),
                'scrollable'  => array(
                    'type'      => 'checkbox',
                    'label'     => __('Limit the height of items list (scrollable)', 'togo-framework'),
                    'condition' => array(
                        'source!'  => array('product_status'),
                        'display' => array('list', 'checkboxes'),
                    ),
                ...
                'show_empty_cats'  => array(
                    'type'      => 'checkbox',
                    'label'     => __('Show empty categories', 'togo-framework'),
                    'condition' => array(
                        'source'  => array('product_cat'),
                    ),
                ),
                'disable_onsale'  => array(
                    'type'      => 'checkbox',
                    'label'     => __('Disable on sale', 'togo-framework'),
                    'condition' => array(
                        'source'  => array('product_status'),
                    ),
                ),
                'disable_in_stock'  => array(
                    'type'      => 'checkbox',
                    'label'     => __('Disable in stock', 'togo-framework'),
                    'condition' => array(
                        'source'  => array('product_status'),
                    ),
                ),
        ...
        public function admin_scripts($hook)
        {
            ...
            wp_enqueue_style('togo-products-filter-admin', TOGO_FRAMEWORK_PATH . '/inc/widgets/assets/css/products-filter-admin.css', array(), null);
            wp_enqueue_script('togo-products-filter-admin', TOGO_FRAMEWORK_PATH . '/inc/widgets/assets/js/products-filter-admin.js', array('wp-util'), null, true);

            wp_localize_script(
                'togo-products-filter-admin',
                'togo_products_filter_params',
                array(
                    'sources'    => $this->get_filter_source_options(),
        ...
        public function scripts()
        {
            ...
            wp_enqueue_script('togo-products-filter', TOGO_FRAMEWORK_DIR . '/inc/widgets/assets/js/products-filter.js', array(
                'jquery',
                'wp-util',
                'select2',
        ...
        public function filter_setting_fields_template()
        {
            ...
            <script type="text/template" id="tmpl-togo-products-filter">
