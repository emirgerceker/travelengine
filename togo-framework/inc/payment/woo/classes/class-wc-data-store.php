 ...
 * @version  3.0.0
 */
abstract class Uxper_Woo_Data_Store_Custom extends WC_Data_Store_WP implements WC_Object_Data_Store_Interface, WC_Product_Data_Store_Interface
{
	...
	protected function read_downloads(&$product)
	{
		...
		if ($meta_values) {
			...
			foreach ($meta_values as $key => $value) {
				...
				$download->set_file(apply_filters('uxper_filter_file_download_path', $value['file'], $product, $key));
				$downloads[] = $download;
			}
	...
	*/
	...
	public function get_on_sale_products()
	{
		...
		return $wpdb->get_results(
			...
			AND posts.post_parent NOT IN (
				...
				WHERE posts.post_type = $this->custom_post_type
	...
	public function is_existing_sku($product_id, $sku)
	{
		...
		return (bool) $wpdb->get_var(
			$wpdb->prepare(
				...
				INNER JOIN {$wpdb->wc_product_meta_lookup} AS lookup ON posts.ID = lookup.product_id
				WHERE
				posts.post_type IN ( $this->custom_post_type )
	...
	public function get_product_id_by_sku($sku)
	{
		...
		return (int) apply_filters('uxper_filter_get_product_id_by_sku', $id, $sku);
	}
	...
	public function get_related_products($cats_array, $tags_array, $exclude_ids, $limit, $product_id)
	{
		...
		$related_product_query = (array) apply_filters('uxper_filter_related_posts_query', $this->get_related_products_query($cats_array, $tags_array, $exclude_ids, $limit + 10), $product_id, $args);
	...
	protected function set_product_stock($product_id_with_stock, $stock_quantity)
	{
		...
		$sql = apply_filters('uxper_filter_update_product_stock_query', $sql, $product_id_with_stock, $stock_quantity, 'set');
	...
	public function update_product_stock($product_id_with_stock, $stock_quantity = null, $operation = 'set')
	{
		...
		$sql = apply_filters('uxper_filter_update_product_stock_query', $sql, $product_id_with_stock, $new_stock, 'set');
	...
	public function search_products($term, $type = '', $include_variations = false, $all_statuses = false, $limit = null, $include = null, $exclude = null)
	{
		...
		$custom_results = apply_filters('uxper_filter_pre_search_products', false, $term, $type, $include_variations, $all_statuses, $limit);

		if (is_array($custom_results)) {
			return $custom_results;
		}

		$post_types   = $include_variations ? array('product', 'product_variation') : array($this->custom_post_type);
		$type_where   = '';
		$status_where = '';
		$limit_query  = '';
		...
		$post_statuses = apply_filters(
			'uxper_filter_search_products_post_statuses',
	...
	protected function get_wp_query_args($query_vars)
	{
		...
		if ('variation' === $query_vars['type']) {
			...
			$wp_query_args['post_type']   = $this->custom_post_type;
		...
		return apply_filters('uxper_filter_data_store_cpt_get_products_query', $wp_query_args, $query_vars, $this);
	}
