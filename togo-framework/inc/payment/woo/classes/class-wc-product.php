 ...
 * @package WooCommerce/Abstracts
 */
abstract class Uxper_Woo_Product_Abstract extends WC_Abstract_Legacy_Product
{
	...
	{
		$this->cache_group = 'uxper-products';
	}
	...
	{
		return $this->get_prop('description', $context) != null ? $this->get_prop('description', $context) : '';
	}
	...
	public function get_dimensions($formatted = true)
	{
		if ($formatted) {
			...
			return apply_filters('uxper_filter_get_dimensions', wc_format_dimensions($this->get_dimensions(false)), $this);
		}
		return array(
	...
	*/
	...
	public function set_catalog_visibility($visibility)
	{
		...
		if (! in_array($visibility, $options, true)) {
			$this->error('product_invalid_catalog_visibility', esc_html__('Invalid catalog visibility option.', 'uxper-booking'));
		}
	...
	public function set_sku($sku)
	{
		...
		if ($this->get_object_read() && ! empty($sku) && ! wc_product_has_unique_sku($this->get_id(), $sku)) {
			...
			$this->error('product_invalid_sku', esc_html__('Invalid or duplicated SKU.', 'uxper-booking'), 400, array('resource_id' => $sku_found));
		}
	...
	public function set_tax_status($status)
	{
		...
		if (! in_array($status, $options, true)) {
			$this->error('product_invalid_tax_status', esc_html__('Invalid product tax status.', 'uxper-booking'));
		}
		...
		foreach ($downloads_array as $download) {
			...
			if (! $download_object->is_allowed_filetype()) {
				if ($this->get_object_read()) {
					...
					$errors[] = sprintf(esc_html__('The downloadable file %1$s cannot be used as it does not have an allowed file type. Allowed types include: %2$s', 'uxper-booking'), '<code>' . basename($download_object->get_file()) . '</code>', '<code>' . implode(', ', array_keys($download_object->get_allowed_mime_types())) . '</code>');
				}
				continue;
			}
			...
			if (! $download_object->file_exists()) {
				if ($this->get_object_read()) {
					...
					$errors[] = sprintf(esc_html__('The downloadable file %s cannot be used as it does not exist on the server.', 'uxper-booking'), '<code>' . $download_object->get_file() . '</code>');
				}
				continue;
			}
	...
	*/
	...
	{
		return apply_filters('uxper_filter_supports', in_array($feature, $this->supports, true), $feature, $this);
	}
	...
	{
		return apply_filters('uxper_filter_is_virtual', true === $this->get_virtual(), $this);
	}
	...
	{
		return apply_filters('uxper_filter_is_sold_individually', true === $this->get_sold_individually(), $this);
	}
	...
	{
		return apply_filters('uxper_filter_is_purchasable', $this->exists() && ('publish' === $this->get_status() || current_user_can('edit_post', $this->get_id()) || current_user_can('owner', $this->get_id())) && '' !== $this->get_price(), $this);
	}
	...
	public function is_on_sale($context = 'view')
	{
		...
		return 'view' === $context ? apply_filters('uxper_filter_is_on_sale', $on_sale, $this) : $on_sale;
	}
	...
	{
		return apply_filters('uxper_filter_is_in_stock', 'outofstock' !== $this->get_stock_status(), $this);
	}
	...
	{
		return apply_filters('uxper_filter_is_taxable', $this->get_tax_status() === 'taxable' && wc_tax_enabled(), $this);
	}
	...
	{
		return apply_filters('uxper_filter_backorders_allowed', ('yes' === $this->get_backorders() || 'notify' === $this->get_backorders()), $this->get_id(), $this);
	}
	...
	*/
	...
	{
		return apply_filters('uxper_filter_get_title', $this->get_name(), $this);
	}
	...
	{
		if ('' === $this->get_price()) {
			$price = apply_filters('uxper_filter_empty_price_html', '', $this);
		...
		return apply_filters('uxper_filter_get_price_html', $price, $this);
	}
	...
	{
		return apply_filters('uxper_filter_add_to_cart_url', $this->get_permalink(), $this);
	}
	...
	{
		return apply_filters('uxper_filter_' . $this->get_object_type() . '_single_add_to_cart_text', esc_html__('Add to cart', 'uxper-booking'), $this);
	}
	...
	{
		return apply_filters('uxper_filter_' . $this->get_object_type() . '_product_add_to_cart_text', esc_html__('Read more', 'uxper-booking'), $this);
	}
	...
	public function add_to_cart_description()
	{
		...
		return apply_filters('uxper_filter_add_to_cart_description', sprintf(esc_html__('Read more about &ldquo;%s&rdquo;', 'uxper-booking'), $this->get_name()), $this);
	}
	...
	public function get_image($size = 'woocommerce_thumbnail', $attr = array(), $placeholder = true)
	{
		...
		return apply_filters('uxper_filter_get_image', $image, $this, $size, $attr, $placeholder, $image);
	}
	...
	public function get_file($download_id = '')
	{
		...
		return apply_filters('uxper_filter_get_file', $file, $this, $download_id);
	}
	...
	public function get_file_download_path($download_id)
	{
		...
		return apply_filters('uxper_filter_get_file_download_path', $file_path, $this, $download_id);
	}
	...
	public function get_price_suffix($price = '', $qty = 1)
	{
		...
		return apply_filters('uxper_filter_get_price_suffix', $html, $this, $price, $qty);
	}
	...
	{
		if (! $this->is_in_stock()) {
			$availability = esc_html__('Out of stock', 'uxper-booking');
		} elseif ($this->managing_stock() && $this->is_on_backorder(1)) {
			$availability = $this->backorders_require_notification() ? esc_html__('Available on backorder', 'uxper-booking') : '';
		} elseif (! $this->managing_stock() && $this->is_on_backorder(1)) {
			$availability = esc_html__('Available on backorder', 'uxper-booking');
		} elseif ($this->managing_stock()) {
			$availability = wc_format_stock_for_display($this);
		} else {
			$availability = '';
		}
		return apply_filters('uxper_filter_get_availability_text', $availability, $this);
	}
	...
	protected function get_availability_class()
	{
		...
		return apply_filters('uxper_filter_get_availability_class', $class, $this);
	}
}
